        ...
        for i in range(self.ou_lookback, n):
            ...
            # ---------------- 4. SOLO SURVIVOR STATE ----------------
                ...
                if long_active:
                    ...
                    # Take profit, cut loss, trailing stop, or mean-reversion target reached
                    if pnl >= self.take_profit_pct or pnl <= -self.cut_losing_leg_pct or drawdown >= self.trailing_stop_pct or z >= 0.0:
                        long_active = False
                        state = "FLAT"
                elif short_active:
                    pnl = (short_entry - price) / short_entry
                    ...
                    # Take profit, cut loss, trailing stop, or mean-reversion target reached
                    if pnl >= self.take_profit_pct or pnl <= -self.cut_losing_leg_pct or drawup >= self.trailing_stop_pct or z <= 0.0:
                        short_active = False
                        state = "FLAT"
            ...
            if not (np.isnan(z) or price <= 0):
                ...
                # 4. SOLO SURVIVOR STATE
                    ...
                    if self.long_active:
                        ...
                        if pnl >= tp_pct or pnl <= -cut_losing_pct or drawdown >= trail_stop_pct or z >= 0.0:
                            self.long_active = False
                            self.state = "FLAT"
                    elif self.short_active:
                        pnl = (self.short_entry - price) / self.short_entry
                        ...
                        if pnl >= tp_pct or pnl <= -cut_losing_pct or drawup >= trail_stop_pct or z <= 0.0:
                            self.short_active = False
                            self.state = "FLAT"
