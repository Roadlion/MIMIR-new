# backend/app/analytics/user_strategies/rl_tuner.py
"""
Generic RL parameter tuner for user-defined static strategies.
"""
...
if HAS_RL:
    ...
    class GenericParamEnv(gym.Env):
        """Gym env where PPO action is a full param_space assignment for the wrapped strategy."""
        ...
        def __init__(
            ...
            self.param_ranges = [
                template.param_space[n] for n in self.param_names
            ]
        ...
        def _decode_action(self, action) -> dict:
            return {
                ...
                for name, (lo, hi), a in zip(
                    self.param_names, self.param_ranges, action
                )
            }
        ...
        def _get_obs(self):
            ...
            dd = (
                (self.balance - self.peak_balance) / self.peak_balance
        ...
        def reset(self, seed=None, options=None):
            ...
            self.current_step = (
                min(100, self.n_bars - 1) if self.n_bars > 100 else 0
            )
...
class RLParameterTuner:
    """Trains/loads a PPO meta-controller that retunes any StaticStrategy's param_space."""
    ...
    def __init__(
        ...
        if not HAS_RL:
            raise ImportError(
                "gymnasium and stable_baselines3 are required for RLParameterTuner"
            )
        self.strategy_cls = strategy_cls
        self.base_kwargs = base_kwargs
        self.feature_cols = list(feature_cols)
    ...
    def train(self, df: pd.DataFrame):
        env = GenericParamEnv(
            self.strategy_cls, self.base_kwargs, df, self.feature_cols
        )
        self._model = PPO(
            "MlpPolicy", env, verbose=0, learning_rate=3e-4, n_steps=256
        )
    ...
    def predict_params(self, features: dict, balance_dd: float = 0.0) -> dict:
        ...
        obs = np.nan_to_num(
            np.array(vals, dtype=np.float32), nan=0.0, posinf=0.0, neginf=0.0
        )
        ...
        return {
            name: float(np.interp(a, [-1, 1], [lo, hi]))
            for name, (lo, hi), a in zip(
                template.param_space.keys(),
                template.param_space.values(),
                action,
            )
        }
