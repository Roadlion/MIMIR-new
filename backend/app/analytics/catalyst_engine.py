...
def calculate_ta_execution_bounds(df_prices: pd.DataFrame) -> Tuple[float, float, float]:
    """
    Calculates exact execution parameters from price history:
    - trigger_price: Current close price
    - target_price: 20-day resistance or 1.5x ATR projection
    - stop_loss: 20-day support or 1.0x ATR downside risk
    """
    if df_prices.empty or len(df_prices) < 14:
        last_close = float(df_prices.iloc[-1]['close']) if not df_prices.empty else 100.0
        return last_close, last_close * 1.08, last_close * 0.95
    ...
    support = float(low.rolling(20, min_periods=5).min().iloc[-1])
    resistance = float(high.rolling(20, min_periods=5).max().iloc[-1])

    # Target price: max of resistance level or last_close + 2.0 * ATR
    target_price = max(resistance, last_close + 2.0 * atr)
    # Ensure minimum 5% upside target
    target_price = max(target_price, last_close * 1.05)

    # Stop loss: min of support level or last_close - 1.2 * ATR
    stop_loss = min(support, last_close - 1.2 * atr)
    # Limit max downside risk to 5%
    stop_loss = max(stop_loss, last_close * 0.95)
...
def eval_realtime_article_catalyst(
    ...
    if abs(sentiment_score) < 0.50 or confidence < 0.60:
        return None
    ...
    trigger_price, target_price, stop_loss = calculate_ta_execution_bounds(df_prices)
...
def scan_pre_earnings_catalysts(conn=None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Scan tickers for pre-earnings setup
        ...
        for ticker in tickers:
            ...
            # This is the critical fix: only fire PRE_EARNINGS_BEAT if there is
            # a confirmed upcoming earnings date in 3-10 days.
            # Previously this fired for ANY ticker with positive sentiment regardless
            # of whether earnings were actually coming up (massive false positive source).
            try:
                ...
                if not is_pre_earnings_window(ticker, min_days=3, max_days=10, conn=conn):
                    logger.debug(
                        f"[CATALYST] {ticker}: no upcoming earnings in 3-10 day window — "
                        f"PRE_EARNINGS_BEAT skipped."
                    )
                    continue
            ...
            # ── NARRATIVE FRESHNESS GATE ──────────────────────────────────────
            # Don't fire pre-earnings signal if the bullish narrative is already PEAK/FADING.
            # That means the catalyst is already priced in by the time we see it.
            try:
                from .narrative_tracker import get_narrative_phase
                narr = get_narrative_phase(ticker, conn=conn)
                if narr.phase in ("PEAK", "FADING"):
                    logger.info(
                        f"[CATALYST] {ticker}: PRE_EARNINGS_BEAT skipped — "
                        f"narrative is {narr.phase} (age {narr.narrative_age_hours:.0f}h)."
                    )
                    continue
            except Exception:
                pass
            ...
            avg_score = sent_info["score"]
            avg_conf = sent_info["conf"]
            ...
            try:
                ...
                upcoming = get_upcoming_earnings(days_ahead=10, conn=conn)
            ...
            thesis = (
                ...
                f"TA Execution Bounds: Limit Buy ${trigger_price:.2f} | Take-Profit Target ${target_price:.2f} (+{((target_price/trigger_price)-1)*100:.1f}%) | Stop ${stop_loss:.2f} (-{((1-(stop_loss/trigger_price))*100):.1f}%)"
            )
            ...
            reason_str = f"PRE_EARNINGS_BEAT: Pre-earnings sentiment score +{avg_score:.2f} | Bullish channel accumulation into report."
